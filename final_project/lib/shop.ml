...
(* let joker_purchase () = let joker_list = load_file "../data/jokers.txt" in
   let rand_idx = Random.int (List.length joker_list) in let random_joker =
   List.nth joker_list rand_idx in Joker.of_string random_joker *)
...
let rec generate_joker (jokers : Joker.t array ref) (rarity : int) =
  if rarity <= 70 then
    let common_jokers = load_file "../data/common_jokers.txt" in
    let random_joker =
      Joker.of_string
    ...
    let uncommon_jokers = load_file "../data/uncommon_jokers.txt" in
    let random_joker =
      Joker.of_string
    ...
    let rare_jokers = load_file "../data/rare.txt" in
    let random_joker =
      Joker.of_string
    ...
    let legendary_jokers = load_file "../data/legendary.txt" in
    let random_joker =
      Joker.of_string
...
let open_shop (money : int ref) (deck : Deck.t ref) (jokers : Joker.t array ref)
  ...
  while !shopping do
    ...
    if !planet_card_bought then print_endline "Planet card already purchased!"
      ...
      print_endline ("3. " ^ PlanetCard.to_string !random_planet_card ^ " - $3");
